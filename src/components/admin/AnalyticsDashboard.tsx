    ...
    <div className="min-h-screen bg-graphite-50">
      ...
      <div className="flex h-screen">
        ...
        <aside className="flex w-56 flex-shrink-0 flex-col border-r border-graphite-200 bg-white">
          ...
          <div className="border-t border-graphite-100 px-5 py-3 text-[11px] text-graphite-500">
            {profile.nome ?? "Admin"}
            {profile.lider && (
              <span className="ml-2 inline-block rounded bg-gold-100 px-1.5 py-0.5 text-gold-700">
                líder
              </span>
            )}
          </div>
          ...
          <div className="mx-auto max-w-6xl px-6 py-6">
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-0.5 text-sm text-graphite-500">
            ...
            {!data || !kpis ? (
              <Card>
                <div className="py-8 text-center text-sm text-graphite-500">
              ...
              <>
                ...
                <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
                  ...
                  <KpiCard
                    ...
                    value={kpis.tempo_medio_horas !== null ? `${kpis.tempo_medio_horas}h` : "—"}
                    tone="info"
                  />
                  <KpiCard
                    label="SLA 24h"
                    value={kpis.sla_24h_pct !== null ? `${kpis.sla_24h_pct}%` : "—"}
                    tone={kpis.sla_24h_pct !== null && kpis.sla_24h_pct >= 80 ? "success" : "warn"}
                  />
                  ...
                  <KpiCard
                    label="Em risco (SLA)"
                    value={kpis.em_risco}
                    tone={kpis.em_risco > 0 ? "warn" : "success"}
                  />
                  <KpiCard
                    label="Vencidos (SLA)"
                    value={kpis.vencidos}
                    tone={kpis.vencidos > 0 ? "danger" : "success"}
                  />
                </div>

                {/* Linha do tempo: chamados por mês */}
                <Card className="mb-4">
                  <div className="mb-3 text-sm font-semibold text-graphite-900">
                ...
                <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                  {/* Donut por status */}
                  <Card>
                    <div className="mb-3 text-sm font-semibold text-graphite-900">Por status</div>
                    {porStatus.length === 0 ? (
                      <div className="py-12 text-center text-sm text-graphite-500">Sem dados.</div>
                    ) : (
                  ...
                  {/* Bar por classe */}
                  <Card>
                    <div className="mb-3 text-sm font-semibold text-graphite-900">Por classe</div>
                    {porClasse.length === 0 ? (
                      <div className="py-12 text-center text-sm text-graphite-500">Sem dados.</div>
                    ) : (
                      <div style={{ width: "100%", height: 240 }}>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <Card>
                    ...
                    {topUnidades.length === 0 ? (
                      <div className="py-6 text-center text-sm text-graphite-500">Sem dados.</div>
                    ) : (
                      <ul className="space-y-2">
                        {topUnidades.map((u) => (
                          <li key={u.unidade} className="flex items-center justify-between text-sm">
                            <span className="mr-2 flex-1 truncate text-graphite-700">
                              {u.unidade}
                            </span>
                            <span className="font-semibold text-graphite-900">{u.total}</span>
                          </li>
                        ))}
                      </ul>
                    )}
                      ...
                      <ul className="space-y-2">
                        {topAutores.map((a) => (
                          <li key={a.nome} className="flex items-center justify-between text-sm">
                            <span className="mr-2 flex-1 truncate text-graphite-700">{a.nome}</span>
                            <span className="font-semibold text-graphite-900">{a.total}</span>
                          </li>
                        ))}
                      </ul>
                    )}
...
function SidebarItem({ active, label, href }: { active?: boolean; label: string; href: string }) {
...
function KpiCard({
  ...
  return (
    <div className={`rounded-lg border p-4 ${toneClasses[tone]}`}>
      <div className="text-[11px] font-semibold uppercase tracking-wide text-graphite-600">
        {label}
      </div>
